...
extension TableCellNodeAttributesExtension on Node {
  double get cellWidth {
    ...
    return attributes[TableCellBlockKeys.width]?.toDouble() ?? TableDefaults.colWidth;
  }

  double get cellHeight {
    assert(type == TableCellBlockKeys.type);
    return attributes[TableCellBlockKeys.height]?.toDouble() ?? TableDefaults.rowHeight + 10;
  }

  double get colHeight {
    assert(type == TableBlockKeys.type);
    return attributes[TableBlockKeys.colsHeight]?.toDouble() ?? TableDefaults.rowHeight + 10;
  }
}
